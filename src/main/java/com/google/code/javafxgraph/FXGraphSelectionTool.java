...
public class FXGraphSelectionTool extends FXTool {
	...
	private double grid = DEFAULT_GRID;
	...
	public void updateSelectionInScene() {
		...
		if (currentSelection.size() > 0) {
			...
			theBuilder.x(startX).y(startY).width(width).height(height).strokeWidth(1).stroke(Color.BLACK)
					.strokeDashArray(3.0, 7.0, 3.0, 7.0).fill(Color.TRANSPARENT).mouseTransparent(true);
			...
			currentSelectionTimeline = TimelineBuilder.create().cycleCount(Animation.INDEFINITE).keyFrames(theOneFrame)
					.build();
	...
	public void startSelectionAt(double aSceneX, double aSceneY) {
		RectangleBuilder theBuilder = RectangleBuilder.create();
		theBuilder.x(aSceneX).y(aSceneY).width(1).height(1).strokeWidth(1).stroke(Color.BLACK)
				.strokeDashArray(3.0, 7.0, 3.0, 7.0).fill(Color.TRANSPARENT).mouseTransparent(true);
		...
		interactiveSelectionTimeline = TimelineBuilder.create().cycleCount(Animation.INDEFINITE).keyFrames(theOneFrame)
				.build();
	...
	/**
	 * dragging the mouse will take effect within the grids only.
	 */
	@Override
	public void mouseDragged(MouseEvent aEvent) {
		if (mousePressedOnNodeOrSelection || pressedWaypoint != null) {
			double x = getNextGridOf(aEvent.getSceneX());
			double y = getNextGridOf(aEvent.getSceneY());
			if (!dragging) {
				dragging = true;
			} else {
				double movementX = x - lastDragX;
				double movementY = y - lastDragY;
	...
	private double getNextGridOf(double position) {
		double mod = position % grid;
		double griddedPosition = mod == 0 ? position : (position - mod);
		return griddedPosition;
	}

	public final void setGrid(double grid) {
		this.grid = grid < 1 || grid == Double.POSITIVE_INFINITY ? DEFAULT_GRID : grid;
	}
}